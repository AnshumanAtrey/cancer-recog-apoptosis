...
- **A6 Autonomous intracellular AND-gate (MHC-free, Shriya's original)** — synthetic gene circuit: intracellular signal A AND B → self-destruct, no MHC, for the dark core. `Test:` all pairs of 8 transcriptional programs on real single-cell data (309k vital + 153k tumour cells), worst-donor vital leak. **Status: ✅ RUNG-23 — DECISIVE NEGATIVE: 0 gates safe&effective. Best coverage ~15% leaks 64%; the "clean" gates cover ~1–3%. Proliferation/glycolysis/MYC are high in REGENERATING vital tissue (marrow) too → AND doesn't escape it → same specificity-coverage anti-correlation as surface. UNIFYING INSIGHT: every EXPRESSION window (surface RUNG-15 + intracellular RUNG-23, single + AND) LEAKS → the MUTATION (neoantigen) is the ONLY tumour-exclusive signal. CONSTRUCTIVE: an autonomous circuit must sense the MUTATION directly (toehold/CRISPR mutant-sensing), not transcriptional state.**
