...
| **External key** (de novo pMHC binder = immune-route recognizer) | design a binder that's mutation-specific | ⏳ PXDesign BOUNDED (all 4 targets, 0 dual-oracle passers: IDH1/BRAF NULL; PIK3CA 1 single-oracle; KRAS 0/80). **ODesign (the different-generator lever) now GENERATES** — 50 KRAS-G12D binders, 44/50 docked on the mutation (R30b). But the two real bars — **MUT-vs-WT discrimination + dual-oracle certification — remain UNTESTED**. Until they pass, the internal key carries recognition |
...
| **RESISTANCE / EVOLUTIONARY ESCAPE** (does the tumour evolve around the kill?) | escape race + agnostic cross-kill + collateral trap (H10) | ⚠️→✅ ANSWERED (R19/R21/R42) — the bare kill escapes at clinical size (R19, L=μ·N0); the **resistance-AGNOSTIC 2nd killer (R21 NK/wave) is the robust closer** (T+NK+wave 100%). The H10 **collateral-sensitivity trap (R42)** closes the *coupled* escape (curable-ceiling ×1/p_decouple, analytic-validated) but is **blind to the DECOUPLING escape** (new residual **p_decouple**) and **efficacy-dominated by the agnostic killer** at equal budget → a toxicity-sparing COMPLEMENT, not the answer. Robust resistance = R21 + R22 multi-target essential drivers; sharpen by recognising the **driver mutation itself** (min p_decouple) |
...
- ⏳ **KRAS-G12D / A\*11:01** (R30) — *read-the-mutation*. **PXDesign v2 = 80 designs, 0 dual-oracle passers** → BOUNDED at PXDesign (oracles anti-correlate r=−0.41; MUT-vs-WT never triggered). **ODesign (R30b, 2026-06-27) cleared the GENERATION lever:** the all-atom epitope-controlled generator produced **50 designs (5 seeds × 10), all 80-mer, 50/50 unique, 44/50 docked on the G12D Asp (median 3.9 Å)** — the hotspot targeting PXDesign couldn't reliably do. Artifacts in `runs/rung30_kras_g12d/odesign/` (RESULTS.md + outputs.zip + binders.fasta + ranked metrics). **STILL UNTESTED = the two real bars: MUT-vs-WT discrimination + dual-oracle (Protenix×AF2-IG) certification.** Generation ≠ discrimination — do not claim a KRAS binder until the MUT/WT scoring runs.
...
- **Resistance — `p_decouple` (NEW, R42):** how often the tumour can evade the recognised feature WITHOUT losing the driver (decouples escape from cost/collateral-sensitivity). Decides whether the H10 trap holds; minimised by recognising the driver mutation itself. The H10 analogue of leak-correlation ρ.
