...
## ▶ NEXT SESSION — current frontier (after RUNG-42 → ODesign R30b, 2026-07-02)
...
> binder) + the GPU run + the paper. **UPDATE 2026-07-02: the external-binder GPU run is now DONE (R30b) —
> ODesign cleared the generation lever PXDesign couldn't (50 binders, 44/50 docked on the G12D Asp), but
> discrimination is UNTESTED → the new #1 frontier is the MUT-vs-WT discrimination scoring of those binders
> (R30c, P0 below).** Live map = `STATUS.md`; verdicts = `docs/HYPOTHESIS_LEDGER.md`; narrative =
...
- [x] **P1 · External binder via a DIFFERENT generator — ODesign PREPPED, awaiting a GPU run.** ⚡ GPU box / 🌐.
      ...
      **✅ RAN (R30b, 2026-06-27, commit fe6abc6) — CLEARED THE GENERATION LEVER.** Ran on Colab T4 (not the container),
      full sweep 5 seeds × 10 = **50 designs, exit 0, ~21 min.** Structural audit (all 50 CIFs parsed, rule 5): all
      **80-mer, 50/50 UNIQUE** (no mode collapse), target = MUT (peptide `VVVGADGVGK`, pos6=D), **epitope control
      WORKED — 44/50 dock on the G12D Asp (median 3.9 Å), median 4 hotspot-interface residues, 1/50 missed.**
      Artifacts in `runs/rung30_kras_g12d/odesign/`: `RESULTS.md` + `outputs.zip` (50 CIF complexes) +
      `odesign_binders.fasta` (50 seqs) + `odesign_design_metrics.csv` (ranked by hotspot engagement). Working
      notebook + dep/checkpoint recipe (biotite 1.2.0 via --no-deps around protenix's pin; LigandMPNN from IPD; 3
      exact checkpoints): `notebooks/binder_odesign_kras_colab.ipynb` + memory `cancer-recon-apoptosis-project`.
      **Generation ≠ discrimination — the two REAL bars are UNTESTED → the R30c scoring item below.**
