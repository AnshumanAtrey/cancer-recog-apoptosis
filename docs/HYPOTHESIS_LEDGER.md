...
| C7 | **KRAS-G12D/A\*11:01** — read-the-mutation (Gly→Asp presence/absence) | ⏳ PARTIAL/UNDER-POWERED — design **v1 (hotspot B0:6, Extended) = 0/10 on ALL oracles** (best af2_iptm 0.37, af2_ipAE 21 vs ≤10). Difficulty gauge: ≤5% hardest tier on BOTH (same as PIK3CA) → 0/10 at batch=10 is the EXPECTED yield, NOT a refutation. Geometry intact (Asp 30% exposed, presence-vs-absence). **MUT-vs-WT test still PENDING** (no binder grips MUT yet). Next: **v2 large batch (100–200)** | R30c/R32/**R30v1** |
