...
| C7 | **KRAS-G12D/A\*11:01** — read-the-mutation (Gly→Asp presence/absence) | ❌ BOUNDED at PXDesign method (not NULL like IDH1) — **v2 = 80 designs (8 seeds), 0 dual-oracle passers**, 1 Protenix-basic-only. Binders DO engage (af2_iptm 0.70 vs v1 0.37, ptx_iptm 0.86) but **no design passes AF2 bar** (min ipAE 16.1) and the oracles **anti-correlate r=−0.41** (af2>0.5: 16/80, ptx>0.8: 5/80, both: 0) → dual-pass structurally rare, not under-sampled. MUT-vs-WT test never triggers (no confident binder) = **discrimination UNTESTED**. Softer negative than IDH1 (engages but not dual-certifiable). Next lever = a DIFFERENT generator (ODesign/RFdiffusion), not more PXDesign seeds | R30v2 |
...
| H5 | Microenvironment Sniffer (pH/ROS/hypoxia) | ✅ DONE (R40) — NOT a standalone recognizer (leaks ~30%, like all non-mutation windows), but the AND-gate's **independence-provider**: a 30%-leaky INDEPENDENT microenvironment input beats a 5%-clean CORRELATED mutation input. Answers R38's "where do N≥3 independent recognizers come from" → **orthogonal modalities** (mutation+2-HG+microenvironment), joint ≈ product of leaks (~3e-4), 16–83× better than a correlated mutation stack. CONDITIONAL on same-modality correlation (R35 residual): if ρ_within=0, clean stacking wins instead | R40 |
